...
const root = process.env.POS_STATIC_ROOT || join(fileURLToPath(new URL(".", import.meta.url)), "dist");
const upstream = new URL(process.env.POS_API_UPSTREAM || "http://pos-backend:8000");
const port = Number(process.env.PORT || 80);
const maxBodyBytes = parseSize(process.env.POS_CLIENT_MAX_BODY_SIZE || "16m");
...
createServer((req, res) => {
  ...
  console.log(`POS frontend server listening on ${port}`);
});
...
function proxy(req, res, url) {
  ...
  const proxyReq = requestModule(
    {
      ...
      headers,
      timeout: url.pathname.includes("/checkout") ? 120_000 : 60_000,
    },
    (proxyRes) => {
  ...
  proxyReq.on("error", () => {
    ...
    res.end(JSON.stringify({ detail: "POS backend is unavailable" }));
  });
  req.on("data", (chunk) => {
...
function serveStatic(req, res, pathname) {
  ...
  // Older browsers and bookmarks may still request the conventional path.
  if (pathname === "/favicon.ico") {
    const favicon = join(root, "favicon.svg");
    if (existsSync(favicon)) {
      sendFile(res, favicon);
      return;
    }
  }
